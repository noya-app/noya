...
To create a block, first activate the Insert tool <PlusIcon
style={{ display: 'inline', verticalAlign: 'text-top' }} />. You can do this by
clicking the Insert tool button at the top left of the toolbar, or by holding
the `Command ⌘` key (on Mac) or `Control` key (on Windows).
